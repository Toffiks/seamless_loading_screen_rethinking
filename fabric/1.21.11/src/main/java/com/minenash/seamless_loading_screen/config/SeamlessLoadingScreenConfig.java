...
public class SeamlessLoadingScreenConfig {
                ...
                (defaults, config, builder) -> {
                    ...
                    var defaultServerModeOpt = Option.<DisplayMode>createBuilder()
                            ...
                            .controller(opt -> EnumControllerBuilder.create(opt)
                                    ...
                                    .formatValue(val -> Text.translatable("seamless_loading_screen.config.displayMode." + val.name().toLowerCase(Locale.ROOT)))
                            ).build();
    ...
    private static Text formatResolution(ScreenshotResolution resolution) {
        var window = MinecraftClient.getInstance().getWindow();
        ScreenshotResolution.Size size = resolution.resolve(
                window.getFramebufferWidth(), window.getFramebufferHeight());
        return Text.translatable("seamless_loading_screen.config.resolution."
                        + resolution.name().toLowerCase(Locale.ROOT))
                .append(Text.literal(" (" + size.width() + "x" + size.height() + ")"));
    }
