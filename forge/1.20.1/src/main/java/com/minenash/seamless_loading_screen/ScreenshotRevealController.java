...
/** Defines how each reveal mode behaves during the different loading stages. */
public final class ScreenshotRevealController {
    ...
    public float duringChunkLoading(ScreenshotRevealMode mode, float progress, long nowNanos) {
        if (mode == ScreenshotRevealMode.New) {
            return animation.update(clamp(progress), nowNanos);
        }
