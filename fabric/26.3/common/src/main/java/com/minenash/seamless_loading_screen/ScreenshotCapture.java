...
public final class ScreenshotCapture {
    ...
    public static void begin(Runnable continuation, boolean waitForSave) {
        ...
        CompletableFuture.delayedExecutor(CAPTURE_TIMEOUT_SECONDS, TimeUnit.SECONDS).execute(() -> {
            if (job.captured.complete(null)) {
                LOGGER.warn("[SeamlessLoadingScreen] Timed out waiting for a world frame; no screenshot was captured");
    ...
    public static void captureFrame() {
        ...
        try {
            ...
            Screenshot.takeScreenshot(client.gameRenderer.mainRenderTarget(), image -> finishCapture(job, image));
